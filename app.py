...
#--- 2. 侧边栏 ---
with 
st.sidebar: st.header("⚙️ 设置") # 优先读取 Secrets 
  if "GOOGLE_API_KEY" in st.secrets: api_key = st.secrets["GOOGLE_API_KEY"] else: api_key = st.text_input("请输入 Google API Key", type="password")
